...
const RootComponent = (props) => {
  ...
  const [products, setProducts] = useState([
    { id: "p1", title: "Product 1", price: 3000 },
  ...
  function removeProductFromCart(newProduct) {
    let newTotalPrice = cart.totalPrice;

    const newProductList = cart.products.map((product) => {
      if (product.id === newProduct.id && product.qty > 0) {
        product.qty--;
        product.price -= newProduct.price;
        newTotalPrice = cart.totalPrice - newProduct.price;
      }
      return product;
    });
